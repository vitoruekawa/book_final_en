...
MATLABにより潮流計算を行うプログラムを実装する方法を説明する。
潮流計算のためには，式\ref{eq:ohmY2}の連立方程式を解く必要がある。
まずは簡単な例題を用いて，代数方程式の解をMATLABで探索する方法を示す。
...
例\ref{ex:pf3bus}の電力系統モデルに対して潮流計算を行うことを考える。
...
\footnote{電圧フェーザの絶対値と偏角の組を$x$とする場合も同様である。}
。
...
例\ref{ex:simplepowerflow}では潮流計算の実装法を述べた。
この実装では，アドミタンス行列が変更されるたびにプログラム\ref{program:ex2}に相当する関数を書き換える必要がある。
このような書き換えを最小限にするために，プログラムをモジュール群に分割することにより，構造化して見通しよく実装する方法を示す。
...
唯一の引数とする関数を要求する仕様となっているため，プログラム\ref{program:ex3}を直接用いることはできない。
...
8行目において，2つの入力引数のうち，1つの引数のみを残した新たな関数を
生成している。
このカリー化を用いると，グローバル変数を用いることなく
機能の分離を行うことができる。
...
そのインスタンスを活用することにする。
ここでは，異なる種類の母線では異なる制約条件が設定されるが，
「制約条件をもつ」という点では共通であることに注目する。
...
\red{（プログラム1-11ではなく1-10で呼ばれている？あとアンダーバーの前のバックスラッシュは不要？）}
記述が簡略化されている。
...
例えば，
新たな種類の母線を追加する場合にも，\verb|get_constraint|というメソッドをもつ
新たなクラスを実装しさえすれば，プログラム\ref{program:ex4}を変更することなく利用することができる。
言い換えれば，
モジュールの設計者が実装を行うべき範囲を明確化している。
...
\verb|get_admittance_matrix|というメソッドをもつクラスを実装するだけで良い。
このような責任範囲の分離は，プログラムの拡張性を高めるために非常に重要である。
...
例\ref{ex:derY}，例\ref{ex:pf3bus}で扱った3つの母線で構成される電力系統モデルを考えよう。
