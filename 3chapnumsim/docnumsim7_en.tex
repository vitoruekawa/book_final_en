...
\begin{example}[Implementation of Power Flow Calculation using Polymorphism]
...
% \begin{comment}
% In the implementation of Program \ref{program:ex4_main}, constraint conditions
% are calculated by calling a method with a common name, \verb|get_constraint|,
% without being aware of the type of busbar, and the description is simplified.
% Like this, the property of being able to use the same method name regardless of
% the type of instance is called \textbf{polymorphism}, and it is one of the
% important concepts in object-oriented programming.  Using polymorphism, not only
% can the description be simplified, but it also allows for easier addition of new
% classes and more efficient maintenance of programs.
% \end{comment}
...
\begin{example}[Calculation of Admittance Matrix using Object-oriented Programming]
...
\begin{example}[Implementation result of power flow calculation]
...
\begin{example}[Numerical solution method for a simple differential-algebraic
...
\begin{example}[Simple implementation of power system
...
\begin{lstlisting}[language=Matlab, caption=load\_impedance.m, label={program:load}]
...
In these programs, the method \verb|get_nx| returns the number of states and
\verb|get_dx_constraint| returns the time derivative of the state and the
constraint conditions. With the device defined in this way, the
Program\nobreak\ref{program:simple_network_dae} can be rewritten as
...
\begin{example}[Adding a Method to Calculate the Steady State of Generators and Loads]
...
\textbf{backward compatibility}\index{public_reference@backward_compatibility}.
