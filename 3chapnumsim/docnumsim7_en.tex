...
\begin{例}[The power flow calculation of the electrical power system model consisting of two bus bars]\label{ex:2buspf}
Let us consider an electrical power system consisting of two bus bars in \FIGref{fig:2buspf}.
...
\begin{例}[Power flow calculations based on the datasheet]\label{ex:pf3bus}
...
\begin{定理}[Expression of transmission loss through bus bar voltage phasor]
\label{thm:PQ}
...
\begin{補題}[The relationship between the internal state of generators and input/output]\label{lem:delVE}
...
\begin{定理}[The relationship between the internal state of generators and input/output under a steady state]
\label{thm:stst}
...
\begin{定理}[Equivalence generators related to input/output and the internal state]
\label{thm:outst}
...
\begin{例}[Initial response of the electrical power system model]\label{ex:inires}
...
\FIGref{fig:P13ini} shows the initial value response of the frequency deviation corresponding to the two power flow calculations at this time.
...
\begin{例}[Response time of the electrical power system model to changes in load impedance]\label{ex:loadpv}
...
Calculation results are shown in \FIGref{fig:P1load} and \FIGref{fig:P3load}.
...
Changes in the load resistance are about the same for \FIGref{fig:P1load} and \FIGref{fig:P3load}, but the values of generated frequency deviation are different.
...
\begin{例}[Time response of the electrical power system model to bus bar ground fault]\label{ex:busflt}
...
The calculation result is shown in \FIGref{fig:P1fault} and \FIGref{fig:P3fault}. The solid blue line is the frequency deviation of generator 1, and the broken red line is the frequency deviation of generator 3.
\FIGref{fig:P1fault} shows that the oscillation of the frequency of generator 3 is large under the steady power flow distribution of \ref{table:pflow1}.
...
On the other hand, with the steady power flow distribution of \TABref{table:pflow2}, frequency oscillation is small even for the same ground fault (\FIGref{fig:P3fault}).
...
\begin{定義}[Synchronization of the steady power flow distribution and bus bar voltage]
\label{def:sync}
...
\FIGref{fig:centerg} shows the relationship when bus bar $1$ is connected to bus bar $2$ and bus bar $3$. Focusing on this fact, the following result can be derived.
...
\begin{補題}[Synchronization of bus bars derived from the balance equation of electric power]
\label{lem:sumc2}
...
Lemma \ref{lem:sumc2} shows that when the degree of the bus bar of interest is 1; in other words, for end-point bus bars such as \FIGref{fig:bussync}(a), bus bars adjacent to such bus bars synchronize.
In addition, if the degree of the bus bar of interest (the node shown with a thick line) is 2; in other words, for bus bars in a chain such as \FIGref{fig:bussync}(b), at least on bus bar on each side, are synchronized.
Furthermore, when the degree of the bus bar of interest is 3; meaning, a bus bar of a node is connected with three transmission lines as in \FIGref{fig:bussync}(c), at least one adjacent bus bar synchronizes with the bus bar of interest.
...
\begin{定理}[Synchronization of bus bars in a tree-structured power grid]
\label{thm:tree}
...
\begin{証明}
\red{Translated with DeepL}
Note the bus bar at the endpoint indicated by the bold line in \FIGref{fig:treepr}(a).
...
By repeating this process, as in \FIGref{fig:treepr}(a), synchronization of all the bus bars is shown in the chain path connecting the endpoint and the bus bars of nodes of degree 3 or more.

Similarly, since all the bus bars in a node of degree 3 or more from another endpoint are synchronous, we see that all the bus bars in all chain paths connected to the bus bar in the node indicated by the bold line, as in \FIGref{fig:treepr}, are all synchronous.
...
\begin{定理}[Synchronization of bus bars in a power grid with a ring structure]
\label{thm:circ}
...
\begin{例}[Synchronization of four bus bars in a power grid with a ring structure]\label{ex:symbox}
Let us consider synchronization of bus bars under a steady power flow distribution in a power grid with a ring structure shown in \FIGref{fig:4busbox}.
...
However, as it is shown with red and blue in \FIGref{fig:4busbox}, synchronization of alternating bus bars is shown by Lemma \ref{lem:sumc2}.
...
These two conditions indicate that only when the power grid of \FIGref{fig:4busbox} has specific symmetry related to the admittance value, a condition arises where only alternating bus bars synchronize.
\end{例}
...
\begin{例}[Searching for a solution for algebraic equations]\label{ex:algenraic_equation_ex1}
...
\begin{例}[Implementation method of power flow calculations]\label{ex:simplepowerflow}
...
\begin{例}[Separation of the implementation of the admittance matrix]
...
\begin{例}[Separation of bus bar implementation]
...
\begin{例}[Object-oriented computation of admittance matrices]
...
\begin{例}[Tidal current calculation implementation results]
\red{Translated with DeepL}
...
\begin{例}[Numerical solution of simple systems of differential-algebraic equations]\label{ex:dae_ex1}
\red{Translated with DeepL}
Numerical simulation is performed for the simple electric circuit shown in \FIGref{fig:RLC}, where $R$, $L$, $C$, and $E$ are all 1.
...
\begin{例}[Simple implementation of power system simulation]\label{ex:dae_ex2}
...
\begin{例}[Modularization of generators and loads]\label{ex:gen_load}
...
\begin{例}[Add methods to calculate steady state conditions for generators and loads]
...
\begin{例}[Numerical calculation of time response to ground fault]
...
\begin{例}[Numerical computation of time response to input signals]
