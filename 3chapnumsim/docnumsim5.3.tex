...
\ref{ch:model}章では，発電機モデルや負荷モデルが送電網モデルで結合されることにより，電力系統全体の数理モデルが非線形微分代数方程式で記述されることを説明した。
...
\bigr)
\end{align}
を時刻$t$での\textbf{潮流状態}（\red{power flow distribution}）と呼ぶ。
...
これは，送電線のコンダクタンス成分（レジスタンス成分）により送電損失が生じるため，式\ref{eq:P1P2ex}の設定では電力の需給が系統全体でバランスしないことを表している。
...
一般に電気学会標準モデル，IEEE 39母線系統モデル\cite{athay1979practical}，IEEE 68母線系統モデル\cite{singh2013ieee}などには，各送電線のインピーダンス値に加えて，各発電機母線に供給される電力や各負荷母線で消費される電力の標準的な値がデータシートとして与えられている。
...
参考として，3つの発電機母線（母線1--母線3）と2つの負荷母線（母線4，母線5）から構成される電力系統モデルの例を\FIGref{fig:powerflow}に示す。
...
\end{align*}
と設定する。
ここで，母線2と母線3を結ぶ送電線のコンダクタンス（$\bm{y}_{23}$の実部）は0に設定されており，その送電線における有効電力の送電損失は0となることに注意されたい。
...
\]
となる。
先の例と比較して，有効電力の送電損失が約25倍となることがわかる。
...
一般に，コンダクタンス成分（レジスタンス成分）が大きい送電線を使って消費電力を供給する場合に，有効電力の送電損失が大きくなる。
送電損失が大きくなるほど，同じ有効電力の消費量を供給するために必要となる有効電力の発電量が大きくなるため，発電費用などの経済コストも大きくなってしまう。
...
本書では，平衡点の選び方と安定度の関係を\ref{ch:stabcont}章で詳しく議論する。

\subsection{潮流計算の実装法}

\red{（MATLABによる実際のコーディング方法）}
...
別の言い方をすれば，送電線の対地静電容量を大きくすることや，コンデンサの特性をもつ機器を母線に接続することは，無効電力の損失を減らす効果を与えることがわかる。
...
潮流計算により定められた有効電力と無効電力の値を$P^{\star}$と$Q^{\star}$，電圧フェーザの絶対値を$|\bm{V}^{\star}|$と表せば，負荷のインピーダンス$\bm{z}_{\rm load}^{\star}$の実部（レジスタンス）と虚部（リアクタンス）は
...
定理\ref{thm:outst}が示すように，発電機への外部入力$(P_{\rm mech},V_{\rm field})$が定数であり，かつ，内部状態$(\delta,E)$が定常状態にあることと，母線に対する入出力$(P,Q,|\bm{V}|,\angle \bm{V})$が定常状態にあることが等価である。
...
\medskip
 ...
 \centering
  \begin{tabular}{|c|c|c|c|c|c|c|}
   \hline
 &  $P_{{\rm mech}i}^{\star}$~[pu] & $V_{{\rm filed}i}^{\star}$~[pu] & $\delta_i^{\star}$~[rad] & $\Delta \omega_i^{\star}$~[pu] & $E_i^{\star}$~[pu] \\
   \hline \hline
   発電機1 & $0.5$ & 2.0442 & 0.0670 & 0 & 2.0210 \\
   \hline
   発電機3 & $2.5006$ & 2.5062 & 0.3870 & 0 & 2.2097 \\
   \hline
  \end{tabular}
\end{table}
...
\medskip
 ...
 &  $P_{{\rm mech}i}^{\star}$~[pu] & $V_{{\rm filed}i}^{\star}$~[pu] & $\delta_i^{\star}$~[rad] & $\Delta \omega_i^{\star}$~[pu] & $E_i^{\star}$~[pu] \\
   \hline \hline
   発電機1 & $2.5158$ & 2.7038 & 0.5356 & 0 & 2.3069 \\
   \hline
   発電機3 & $0.5$ & 2.1250 & 0.0390 & 0 & 2.0654 \\
   \hline
  \end{tabular}
\end{table}
...
とする。
ただし，周波数偏差の初期値は0とする。
このとき，2つの潮流計算結果に対する周波数偏差の初期値応答を\FIGref{fig:P13ini}に示す。
...
%\caption{データシートと潮流計算結果(1)} \label{table:pflow1}
 \centering
  ...
  \begin{tabular}{|c|c|c|c|c|c|c|}
   \hline
 &  $\real [\bm{z}_{{\rm load}2}^{\star}]$ & $\imag [\bm{z}_{{\rm load}2}^{\star}]$ \\
   \hline \hline
   定常 & 1.3293 & 0 \\
   \hline
   増加 & 1.3426 & 0 \\
...
具体的には，潮流計算の結果から逆算された負荷のレジスタンスを1\%増加または減少させて時間応答を計算する。
...
また，レジスタンスが増加すると有効電力の消費が一般に大きくなるため，発電機の周波数は低下する。
レジスタンスが減少する場合は逆である。
発電機の機械的トルクがレジスタンスが変化する前の値で固定されているため，有効電力の需給バランスが取れなくなり，周波数偏差の定常値は非零となっていることに注意されたい。
なお，負荷のレジスタンスの変化量は\FIGref{fig:P1load}と\FIGref{fig:P3load}の両者でほぼ等しいが，生じる周波数偏差の値は異なっている。
...
\begin{figure}[t]
  ...
  \begin{minipage}{0.49\linewidth}
    ...
    \subcaption{ レジスタンスが1\%増加した場合 }
  \end{minipage}
  \begin{minipage}{0.49\linewidth}
    \centering
    \includegraphics[width = 1.0\linewidth]{figs/P1pl}
    \subcaption{レジスタンスが1\%減少した場合 }
  \end{minipage}
  \medskip
...
以下では，$N$個の母線で構成される電力系統モデルに対して，一般性を失うことなく，母線$N$に地絡が発生するものとして説明する。
また，地絡は時刻0~[s]に発生し，時刻$t_0$~[s]まで継続するものとする。
このとき，式\ref{eq:pfconIV}の潮流状態は，$t<0$と$t\geq t_0$の時刻では，式\ref{eq:ohmY2}の平常時の代数方程式を満たすが，地絡が継続している$ t \in [0, t_0)$の時刻では，地絡母線の電流フェーザ$\bm{I}_{N}(t)$に関する方程式を除いた
...
地絡時に機器$N$から母線$N$に流入する電流フェーザ$\bm{I}_{N}(t)$は，機器$N$の動特性から定められる。
具体的には，機器$N$が発電機である場合には，$ t \in [0, t_0)$の時刻では，$|\bm{V}_{N}(t)|$を0とした
\begin{align}\label{eq:gendynVIfN}
\simode{
\dot{\delta}_N&= \omega_0  \Delta \omega_N\\
M_N   \Delta \dot{\omega}_N &= 
 - D_N \Delta\omega_N  
+P_{{\rm mech}N} 
\\
\taud_N \dot{E}_N & = 
...
このとき，電流フェーザ$\bm{I}_{N}(t)$は，発電機の出力として
\begin{align*}%\label{eq:IoutfN}
|\bm{I}_N(t)|=\tfrac{E_N (t)}{ \Xt_N }
,\qquad
\angle \bm{I}_N (t) = \delta_i (t) - \frac{\pi}{2}
\end{align*}
で与えられる。
同様に，機器$N$が定インピーダンスの負荷モデルである場合には，式\ref{eq:cimp}の$|\bm{V}_{N}(t)|$が0であるため，$|\bm{I}_{N}(t)|$も0となる。
他の負荷モデルの場合も基本的に同様であるが，定電力の負荷モデルでは$|\bm{I}_{N}(t)|$が無限大となるため，数値シミュレーションでは数値的な不安定性に注意する必要がある。
なお，母線$N$から大地に流れる地絡電流フェーザは
\[
\bm{I}_{N}'(t) := \bm{I}_{N}(t) - \sum_{j=1}^{N-1} \bm{Y}_{Nj} \bm{V}_{j}(t),\qquad
 t \in [0, t_0)
\]
と表すことができる。
