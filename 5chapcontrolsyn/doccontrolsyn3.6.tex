...
ただし，コントローラのパラメータは\ref{table:agcpara}(a)の値に設定している。
...
なお，母線の地絡時には$|\bm{V}|$が0となるため，$V_{\rm field}$は出力されない。
...
\end{align}
と表せる。
このモデルのパラメータ例を\ref{table:AVRparast1}の例3と例4に示す。

なお，式\ref{eq:avrst1}や式\ref{eq:avrst1s}の参照信号$V_{{\rm ref}}^{\star}$の値は，所望の界磁電圧の定常値$V_{{\rm field}}^{\star}$と母線電圧の絶対値の定常値$|\bm{V}^{\star}|$が与えられるとき
...
式\ref{eq:basenvm}の行列$\overline{\mathit{\Theta}}^{\rm int}$により，発電機の内部状態がそれ自身に与える局所的なフィードバック構造をモデル化することができる。
...
によりモデル化できる
\footnote{
電力系統は常に安定に運用されている必要があるため，式\ref{eq:ThetaV}の$\psi$は運用しながら計測されたデータを
用いて同定する必要がある。
...
また，式\ref{eq:ThetaV}の$\hat{\psi}$は$\psi$の同定された値を表している。
...
例\ref{ex:avreffect}と同様の3母線で構成される電力系統モデルにおいて，発電機1と発電機3に同一の自動電圧調整器が組み込まれている場合を考えよう。
なお，定常潮流状態は，\ref{table:pflow2}の潮流計算結果に対応するものとする。
以下では，発電機1に注目して，式\ref{eq:thetaV}の$\psi_1$の要素である偏微分の値を計測データから同定する。
ただし，$\tfrac{\partial |\bm{V}_1|}{\partial \delta_1}(z_{\mathds G}^{\star})$と$\tfrac{\partial \angle \bm{V}_1}{\partial E_1}(z_{\mathds G}^{\star})$の大きさは一般に小さいため
\[
\theta_1:=
\frac{\partial | \bm{V}_1|}{\partial E_1}(z_{\mathds G}^{\star})
,\qquad
\theta_2:=
\frac{\partial \angle \bm{V}_1}{\partial \delta_1}(z_{\mathds G}^{\star})
\]
の2つの値のみをデータから同定する。
このとき，式\ref{eq:ThetaV}の$\overline{\mathit{\Theta}}^{\rm ext}_1$は
\begin{align}\label{eq:Thext1para}
\overline{\mathit{\Theta}}^{\rm ext}_1=
\mat{
\tfrac{E_1^{\star}|\bm{V}_1^{\star} |}{ \Xt_1 } \Delta^{\sf cos}_1 \theta_2
& 
0
&
- \tfrac{ E_1^{\star} }{ \Xt_1 } \Delta^{\sf sin}_1 \theta_1
\\
\left( \tfrac{ \Xs_1 }{ \Xt_1 }-1 \right)
|\bm{V}_1^{\star}| \Delta^{\sf sin}_1 \theta_2
&
0
&
\left( \tfrac{ \Xs_1 }{ \Xt_1 }-1 \right)
|\bm{V}_1^{\star}| \Delta^{\sf cos}_1 \theta_1
\\
0 & 0 & \theta_1
}
\end{align}
とパラメータ化される。
ただし，定数$\Delta^{\sf sin}_1$，$\Delta^{\sf cos}_1$は
\[
\Delta^{\sf sin}_1 := \sfsin(\delta_1^{\star} -  \angle \bm{V}_1^{\star}) ,\qquad
\Delta^{\sf cos}_1 := \sfcos (\delta_1^{\star} - \angle \bm{V}_1^{\star} )
\]
で定義される。

パラメータ$(\theta_1,\theta_2)$の最適化は以下のように行う。
自動電圧調整器の入力$V_{{\rm pss}1}$を無作為に励振した場合に得られる$(\delta_1,E_1)$の時系列データを電力系統モデルから取得する。
また，発電機1に対して，式\ref{eq:desmodl}の局所線形サブシステム$G_1$と近似線形環境モデルのパラメータ
$\overline{\mathit{\Theta}}_1$のフィードバック系として，式\ref{eq:Gplus}の$G_{1+}$を構成する。
ただし，$\overline{\mathit{\Theta}}_1$は，発電機1に対する式\ref{eq:basenvm}の$\overline{\mathit{\Theta}}_{1}^{\rm int}$と式\ref{eq:Thext1para}の$\overline{\mathit{\Theta}}_{1}^{\rm ext}$の和とする。
このとき，式\ref{eq:Gplus}の入力$\hat{u}_1$として$V_{{\rm pss}1}$の励振信号を印加した場合の$\hat{y}_1$の第1要素，第3要素である$(\hat{\delta}_1,\hat{E}_1)$の時系列データが得られる。
パラメータ$(\theta_1,\theta_2)$は，データを取得した時間区間におけるすべての時刻$t$に対して
\[
\delta_1(t) \simeq \hat{\delta}_1(t)
,\qquad
E_1(t) \simeq \hat{E}_1(t)
\]
となるように最適化する。
%明らかに，$(\hat{\delta}_1,\hat{E}_1)$の時系列データは，$(\theta_1,\theta_2)$の関数となる。

\FIGref{fig:timeVmodel}の青線として，0~[s]から100~[s]の間で$V_{{\rm pss}1}$を無作為に励振した場合の$(\delta_1,E_1)$の時系列データを示す。
このデータに対して最適なパラメータ$(\theta_1,\theta_2)$を幅0.01の等間隔グリッドで網羅的に探索した結果を\FIGref{fig:datamodeling}に示す。
横軸と縦軸はそれぞれ，設定した$\theta_1$と$\theta_2$の値を表しており，(a)と(b)の領域の色はそれぞれ，誤差$Q(\delta_1,\hat{\delta}_1)$，$Q(E_1,\hat{E}_1)$の値を表している。
ただし
\[
Q(x,\hat{x}):=
\sum_{k=1}^{10000}
...
\right\|^2
\]
は，$t\in [0,100]$の連続時間信号$x(t)$，$\hat{x}(t)$を0.01~[s]の周期でサンプリングした10,000点の離散時間信号に対して誤差を評価する関数である。
この結果に基づき，パラメータの値を$(xxxx,xxxx)$に設定する。
このパラメータに対する$(\hat{\delta}_1,\hat{E}_1)$の時系列データを\FIGref{fig:timeVmodel}の赤破線で示す。
\red{......であることがわかる。}
\end{例}
