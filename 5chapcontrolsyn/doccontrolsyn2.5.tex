...
\end{align}
である。
これは，発電機バスに関するサブシステムのように見えるが，式\ref{eq:sys2}におけるバスの電圧フェーザは，すべての発電機バス$i \in \mathcal{I}_{\rm G}$に関する連立方程式
...
\ref{sec:linpasana}節における近似線形モデルの設定に合わせて，すべてのバスに発電機が接続されている場合を考える
\footnote{
...
\blue{過渡安定性などとの用語の関係？同期安定性と同期安定化？
AVRの必要性や目的、PSSの必要性や目的など}

\subsection{自動電圧調整器と系統安定化装置}\label{sec:avrpss}
