...
本節では，未知の負荷変動に対して適切に発電量を調整するための\textbf{自動発電制御}（AGC：Automatic Generation Control）\index{ じどうはつでんせいぎょ@自動発電制御}の動作原理を説明する。
...
電力系統工学では，角周波数偏差を0に漸近収束させる制御全般を総称して\textbf{周波数安定化制御}（frequency stabilization control）\index{ しゅうはすうあんていかせいぎょ@周波数安定化制御}と呼ぶ。
...
\end{subequations}
である。
電力系統工学では，非負定数$\alpha_i$は発電機$i$の\textbf{寄与係数}（participation factor）\index{ きよけいすう@寄与係数}と呼ばれる。
現実の火力発電や原子力発電では，火力や原子力で発生させた高圧の蒸気によりタービンを回転させて
機械入力を発生させる\textbf{原動機}（prime mover）\index{ げんどうき@原動機}が存在することに注意されたい。
原動機には，発電機の回転速度を自動制御する\textbf{ガバナ}（governor）\index{ がばな@ガバナ}が組み込まれている。
...
電力系統工学の用語では，この寄与係数を更新するスキームは，\textbf{経済負荷配分制御}（EDC：Economic load Dispatching Control）\index{ けいざいふかはいぶんせいぎょ@経済負荷配分制御}と呼ばれる。
また，寄与係数を更新間隔の間で定数として用いる制御アルゴリズムは，\textbf{負荷周波数制御}（LFC：Load Frequency Control）\index{ ふかしゅうはすうせいぎょ@負荷周波数制御}と呼ばれる。
...
発電機や送電線の物理定数は例\ref{ex:inires}と同じ値に設定し，発電機の内部状態の初期値には，\TABref{table:genst13b}の定常値を設定する。
また，界磁入力は\TABref{table:genst13b}の値で一定であるものととする。

母線2の負荷は定電力モデルに設定して，初期値として設定した定常潮流状態から，有効電力の消費量が1\%増加する場合を考える。
まず，自動発電制御が組み込まれておらず，発電機の機械入力が\TABref{table:genst13b}の値で一定である場合の角周波数偏差の時間応答を\FIGref{fig:agcPdemo}(a)に示す。
...
ただし，コントローラのパラメータは\TABref{table:agcpara}(a)の値に設定している。
...
\begin{itemize}
\item 0~[s]から20~[s]の区間では\TABref{table:agcpara}の(b)のパラメータを設定する。
\item 20~[s]から40~[s]の区間では\TABref{table:agcpara}の(a)のパラメータを設定する。
\item 40~[s]から60~[s]の区間では\TABref{table:agcpara}の(c)のパラメータを設定する。
\end{itemize}
...
特に，ブロードキャスト型PIコントローラのパラメータを\TABref{table:agcpara}の(a)の値に設定する場合，すなわち，母線2と母線3を端点とする抵抗のない送電線を主に用いて電力を供給する場合に，電力系統全体の送電損失がより小さくなることがわかる。
...
このような解析を行うためにシステム制御工学で提唱されている概念として，\textbf{平衡点に依存しない受動性}（equilibrium-independent passivity）\index{ へいこうてんにいぞんしないじゅどうせい@平衡点に依存しない受動性}と呼ばれるものがある\cite{hines2011equilibrium,simpson2019equilibrium}。
なお，文献によっては，\textbf{シフトされた受動性}（shifted passivity）\index{ しふとされたじゅどうせい@シフトされた受動性}とも呼ばれている\cite{monshizadeh2019conditions}。
その定義はつぎのように与えられる。
...
\forall t \geq 0
\end{align*}
を満たす微分可能な半正定値関数$W_{x^{\star}}:\mathcal{X} \rightarrow \mathbb{R}_{\geq 0}$が存在するとき，$\Sigma$は\textbf{平衡点に依らず受動的}（equilibrium-independent passive）\index{ へいこうてんによらずじゅどうてき@平衡点に依らず受動的}であると呼ぶ。
...
\forall t \geq 0
\end{align*}
を満たすある正定数$\rho$が存在するとき，$\Sigma$は\textbf{平衡点に依らず強受動的}（equilibrium-independent strictly passive）\index{ へいこうてんによらずきょうじゅどうてき@平衡点に依らず強受動的}であると呼ぶ。
\end{定義}
...
このようなシステム表現は，\textbf{ディスクリプタ形式}（descriptor representation）\index{ でぃすくりぷたけいしき@ディスクリプタ形式}と呼ばれる。
\end{COLUMN}
...
\forall \theta \in [0,1]
\]
が成り立つとき，$f(x)$は\textbf{凸関数}（convex function）\index{ とつかんすう@凸関数}であると呼ぶ。
...
統計学などでは，凸関数である$U(x)$に対して，式\ref{eq:paraW}の右辺の量は$x$と$x^{\star}$の$U(x)$に関する\textbf{ブレグマン距離}（Bregman distance）\index{ ぶれぐまんきょり@ブレグマン距離}と呼ばれている\cite{bregman1967relaxation}。
...
式(\ref{eq:agccon})のブロードキャスト型PIコントローラには，\TABref{table:agcpara}の(b)のパラメータを設定する。

発電機の初期値には，\TABref{table:pflownl}に示される2通りの定常潮流状態に対応する定常値を設定する。
...
\sfdiag \left( \Xti \right) B \sfdiag \left( \Xti \right)
\]
は非対角要素が非正の正定行列であることから，\textbf{M行列}（M-matrix）\index{ えむぎょうれつ@M行列}である。
...
\]
が半正定となることである。
この行列は，関数$f$の\textbf{ヘッセ行列}（Hessian matrix）\index{ へっせぎょうれつ@ヘッセ行列}と呼ばれる\cite{boyd2004convex}。
\end{COLUMN}
...
電力系統工学では，平衡点の安定領域の大きさや安定度の高さを議論する場合に広く\textbf{過渡安定度}（transient stability）\index{ かとあんていど@過渡安定度}という用語が用いられる。
本節では，電力系統の過渡安定度を高める目的で実装される\textbf{励磁系}（excitation system）\index{ れいじけい@励磁系}の数理モデルや特性を概説する。
励磁系は一般に，各発電機に「個別に」実装される局所コントローラであり，発電機が接続されている母線の電圧フェーザや電流フェーザ，発電機の内部状態などを局所的に計測することにより，界磁入力を自動調整する制御動作を行う。
励磁系の主要な要素は，母線電圧を所望の値に維持するための\textbf{自動電圧調整器}（AVR：Automatic Voltage Regulator）\index{ じどうでんあつちょうせいき@自動電圧調整器}と呼ばれる制御機器である。
さらに，自動電圧調整器に起因して励起される動揺を抑制するために，\textbf{系統安定化装置}（PSS：Power System Stabilizer）\index{ けいとうあんていかそうち@系統安定化装置}と呼ばれる追加的な制御アルゴリズムが組み込まれる場合もある。
...
まず，自動電圧調整器の直流型モデルである\textbf{IEEE DC1型モデル}（IEEE Type DC1 excitation system model）\index{ あいとりぷるいーでぃーしー1がたもでる@IEEE DC1型モデル}を説明する。
...
\FIGref{fig:avrdc1}に示されているように，IEEE DC1型モデルは，母線電圧フェーザの絶対値$|\bm{V}|$を入力として，発電機の界磁入力$V_{\rm field}$を出力とする制御器である。
ただし，補足的な入力信号として，母線電圧フェーザの絶対値を所望の値に調整するための参照信号$V_{\rm ref}^{\star}$と系統安定化装置が出力する制御信号$V_{\rm pss}$が印加されている。
自動電圧調整器のIEEE DC1型モデルは，\textbf{計器用変圧器} （voltage transformer）\index{ けいきようへんあつき@計器用変圧器}，\textbf{コンパレータ}（comparator）\index{ こんぱれーた@コンパレータ}，\textbf{増幅器}（amplifier）\index{ ぞうふくき@増幅器}，\textbf{励磁器}（exciter）\index{ れいじき@励磁器}の4つの基本機器に加えて，補助的な\textbf{安定化回路}（stabilizing circuit）\index{ あんていかかいろ@安定化回路}で構成されている。
以下では，それぞれの動特性を説明する。
...
\TABref{table:AVRpara1}と\TABref{table:AVRpara2}に各パラメータの参考値をまとめる。
...
つぎに，IEEE DC1型と同様の構成であるが，より応答が速い静的型モデルである\textbf{IEEE ST1型モデル}（IEEE Type ST1 excitation system model）\index{ あいとりぷるいーえすてぃー1がたもでる@IEEE ST1型モデル}を説明する。
...
パラメータ例を\TABref{table:AVRparast1}の例1と例2に示す。
...
\end{align}
と表せる。
このモデルのパラメータ例を\TABref{table:AVRparast1}の例3と例4に示す。
...
また，母線2に接続されている負荷は定インピーダンスモデルに設定し，そのインピーダンス値には\TABref{table:loadpara1}の1行目の値を用いる。
自動電圧調整器は式\ref{eq:avrst1}のIEEE ST1型モデルに設定する。
発電機1と発電機3に組み込まれる自動電圧調整器は同一であり，パラメータは\TABref{table:AVRparast1}における例1の値を用いる。

まず，自動電圧調整器の有無に対する定態安定な定常潮流状態（安定な平衡点）の集合の変化を確認する。
具体的には，発電機の内部電圧の定常値である$E_1^{\star}$と$E_3^{\star}$は\TABref{table:genst13a}の最右列の値に固定して，回転子偏角の定常値の差である$\delta_3^{\star}-\delta_1^{\star}$をパラメータとして変化させることにより，対応する電力系統の定常潮流状態の定態安定性を近似線形化に基づき判定する。
実際に近似線形モデルが安定となる$\delta_3^{\star}-\delta_1^{\star}$の範囲を計算すると，自動電圧調整器なしの場合には\TABref{table:stableeqs}(i)の範囲となり，自動電圧調整器がある場合には\TABref{table:stableeqs}(ii)の範囲となることがわかる。
...
関数$y:[0,\infty) \rightarrow \mathbb{R}^n$の\textbf{$\mathcal{L}_2$ノルム}（$\mathcal{L}_2$-norm）\index{えるつーのるむ@$\mathcal{L}_2$ノルム}
は
\[
...
\TABref{table:stableeqs}(i)に対応する自動電圧調整器なしの場合を青の実線，(ii)に対応する自動電圧調整器ありの場合を黒の破線で表示している。
...
参考として，$\delta_3-\delta_1$の初期値を$-1$に設定した場合と$-\tfrac{\pi}{2}$に設定した場合に生じる角周波数偏差の時間応答をそれぞれ，\FIGref{fig:avrsmalld}と\FIGref{fig:avrlarged}に示す。
...
以下では，\textbf{IEEE PSS1型モデル}（IEEE Type PSS1 power system stabilizer model）\index{ あいとりぷるいーぴーえすえす1がたもでる@IEEE PSS1型モデル}と呼ばれる標準的な系統安定化装置のモデルを説明する\cite[9.2節]{ieee2016ieee}。
このモデルは主に，\textbf{ウォッシュアウトフィルタ}（washout filter）\index{ うぉっしゅあうとふぃるた@ウォッシュアウトフィルタ}と\textbf{位相進み補償器}（phase-lead compensator）\index{ いそうすすみほしょうき@位相進み補償器}の2つで構成される。
...
このモデルのパラメータ例を\TABref{table:psspara}に示す。
...
発電機1と発電機3に組み込む系統安定化装置は同一であり，パラメータは\TABref{table:psspara}における例2の値を用いる。
まず，系統安定化装置の有無に対する安定な平衡点の集合の変化を確認すると\TABref{table:stableeqs}(iii)の範囲となる。
...
参考として，$\delta_1-\delta_3$の初期値を$-1$に設定した場合と$-1.57$に設定した場合に生じる角周波数偏差の時間応答をそれぞれ，\FIGref{fig:PSSomega}の(a)と(b)に示す。
...
\end{align}
と定義される。
式\ref{eq:desmodl}の$G$を\textbf{局所線形サブシステム}（local linear subsystem）\index{ きょくしょせんけいさぶしすてむ@局所線形サブシステム}と呼ぶ。
...
\]
を設計することを考えよう。
以下では，このコントローラ$K$を\textbf{レトロフィットコントローラ}（retrofit controller）\index{ れとろふぃっとこんとろーら@レトロフィットコントローラ}と呼ぶ。
...
以下では，この推定用モデルを\textbf{近似線形環境モデル}（approximate linear environment model）\index{ きんじせんけいかんきょうもでる@近似線形環境モデル}と呼ぶ。
...
近似線形環境モデルを説明する準備として，\textbf{環境}（environment）\index{ かんきょう@環境}と呼ばれる非線形サブシステムを導入する。
...
\]
と表そう。
このとき，局所線形サブシステム$G$と環境$\overline{G}$のフィードバック結合系は，\FIGref{fig:retconsys}のように「注目する発電機の視点による電力系統全体」を表す。
...
例えば，\textbf{線形2次レギュレータ}（LQR：linear-quadratic regulator）\index{ せんけい2じれぎゅれーた@線形2次レギュレータ}の設計手法\cite[5.3節]{fairman1998linear}を式\ref{eq:Gplus}のコントローラ設計用モデル$G^+$に適用してみよう。
...
\end{align}
を用いる。
ただし，$Q$は半正定，$R$は正定な行列であり，行列$P(\overline{\mathit{\Theta}})$は\textbf{代数リッカチ方程式}（algebraic Riccati equation）\index{ だいすうりっかちほうていしき@代数リッカチ方程式}
\[
...
用いて同定する必要がある。
システム制御工学では，動作しているフィードバック系内のサブシステムの同定は，\textbf{閉ループ同定}（closed-loop identification）\index{ へいるーぷどうてい@閉ループ同定}と呼ばれる\cite{ljung1998system}。
...
%    }
  ...
  \begin{minipage}{0.49\linewidth}
    \centering
    \includegraphics[width = 1.0\linewidth]{figs/timedeltamodel}
    \subcaption{ $\delta_3$と$\hat{\delta}_3$の時系列データ}
     \medskip
  \end{minipage}
  \begin{minipage}{0.49\linewidth}
    \centering
    \includegraphics[width = 1.0\linewidth]{figs/timeEmodel}
    \subcaption{ $E_3$と$\hat{E}_3$の時系列データ
     }
     \medskip
  \end{minipage}
    }
...
また，\TABref{table:agcpara}(b)のパラメータ値が設定された式(\ref{eq:agccon})のブロードキャスト型PIコントローラが自動発電制御として組み込まれているものとする。
なお，定常潮流状態は，\TABref{table:pflow2}の潮流計算結果に対応するものとする。
...
\FIGref{fig:timeVmodel}の青線により，0~[s]から10~[s]の間で$V_{{\rm pss}3}$を無作為に励振した場合の$(\delta_3,E_3)$の時系列データを示す。
ただし，定常値$(\delta_3^{\star},E_3^{\star})$に対する偏差が示されている。
このデータに対して最適なパラメータ$(\theta_1,\theta_2)$を幅0.01の等間隔グリッドで網羅的に探索した結果を\FIGref{fig:datamodeling}に示す。
...
なお，このパラメータ値は\FIGref{fig:datamodeling}の「$\times$」に対応している。
このパラメータ設定に対する$(\hat{\delta}_3,\hat{E}_3)$の時系列データを\FIGref{fig:timeVmodel}の赤破線で示す。
