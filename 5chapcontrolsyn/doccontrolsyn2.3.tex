...
具体的には，任意の$(x,x^{\star}) \in \mathcal{X} \times \mathcal{X}$に対して，
\[
U(x) \geq  U(x^{\star}) + \nabla U^{\sf T}(x^{\star}) (x-x^{\star})
\]
が成り立つならば，各々すべての平衡点$x^{\star} \in \mathcal{E}_{\Sigma}$に対して，$W_{x^{\star}}(x)$は半正定値関数である。
これは，$U(x)$が\emph{凸関数}（convex function）であることを表す
\footnote{
...
\subsubsection{ポテンシャルエネルギー関数が凸となる条件}

以下では，式\ref{eq:potWx}のポテンシャルエネルギー関数$U_{\mathds{G}}(x_{\mathds{G}})$が凸関数となるための条件が，第\ref{sec:linpasana}節の近似線形モデルの受動性解析で現れた受動送電条件(i)と(iii)に対応することを示す。
なお，本節の議論では，受動送電条件(ii)は仮定されている。

第\ref{sec:linpasana}節における近似線形モデルの設定に合わせて，すべてのバスに発電機が接続されている場合を考える
\footnote{
需給が均衡する定常潮流状態においては，一部の発電機は電力を消費するモータとなる。
...
また，縮約サセプタンス$B_{ij}^{\rm red}$は，式(\ref{eq:sys2G})の$B_{ij}$をまとめた送電網のサセプタンス行列を$B$として，
\[
B^{\rm red}
:= -
\bigl\{
...
U_{\mathds{G}}^{\rm red} (x_{\mathds{G}})  := 
 \frac{1}{2} 
\sum_{i\in \mathcal{I}_{\rm G}}
\left\{
...
\end{align}
となる。
ただし，すべての$\delta_i$と$E_i$を並べたベクトルをあらためて$x_{\mathds{G}}$と表している。
内部状態に関する偏微分を計算すると
\begin{align*}
\frac{\partial U_{\mathds{G}}^{\rm red} }{\partial \delta_i}(x_{\mathds{G}})  = y_{\mathds{G}_i}
,\qquad
\frac{\partial U_{\mathds{G}}^{\rm red} }{\partial E_i} (x_{\mathds{G}}) = 
\frac{V_{{\rm field}i}^{\star} - \tau_{{\rm d}i}\dot{E}_i  }{X_{{\rm d}i} - X_{{\rm q}i} }
\end{align*}
が得られる。
同様に，定常状態に対して
\begin{align*}
\frac{\partial U_{\mathds{G}}^{\rm red} }{\partial \delta_i} ( x^{\star}_{\mathds{G}} )
= y_{\mathds{G}_i}^{\star}
,\qquad
\frac{\partial U_{\mathds{G}}^{\rm red} }{\partial E_i} ( x^{\star}_{\mathds{G}} ) = 
\frac{V_{{\rm field}i}^{\star}  }{X_{{\rm d}i} - X_{{\rm q}i} }
\end{align*}
となる。
したがって，対応する蓄積関数を
\[
W_{x^{\star}_{\mathds{G}}}^{\rm red} (x_{\mathds{G}}) = U_{\mathds{G}}^{\rm red} (x_{\mathds{G}}) 
- U_{\mathds{G}}^{\rm red} (x^{\star}_{\mathds{G}}) 
- \left\{ \nabla U_{\mathds{G}}^{\rm red}(x^{\star}_{\mathds{G}}) \right\}^{\sf T}
 (x_{\mathds{G}}-x^{\star}_{\mathds{G}})
\]
と定義すれば，式\ref{eq:disineqGds}と同様に，その時間微分は
\[
\frac{d}{dt}W_{x^{\star}_{\mathds{G}}}^{\rm red} \bigl(x_{\mathds{G}}(t) \bigr)
 \leq 
(y_{\mathds{G}}-y_{\mathds{G}}^{\star})^{\sf T} (u_{\mathds{G}}- u_{\mathds{G}}^{\star})
\]
と評価できる。
...
式\ref{eq:potWxred}の$U_{\mathds{G}}^{\rm red} (x_{\mathds{G}})$が凸関数であることは，ヘッセ行列$\nabla^2 U_{\mathds{G}}^{\rm red} (x_{\mathds{G}})$の半正定性として特徴づけられる
\footnote{
...
\]
が半正定となることである。
この行列は，関数$f$の\emph{ヘッセ行列}（Hessian matrix）と呼ばれる\cite{boyd2004convex}。
}
。
そのヘッセ行列を計算すると
\[
\nabla^2 U_{\mathds{G}}^{\rm red} (x_{\mathds{G}})
=
\mat{
L  &  - \hat{B}^{\sf T} \\
- \hat{B} & -\hat{A}_0
