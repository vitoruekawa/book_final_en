...
\end{align}
と得られていた。
ただし，$\delta$と$E$は，$\delta_i$と$E_i$を並べたベクトルであり，非線形項を
...
\begin{figure}[t]
\centering
\includegraphics[width = .45\linewidth]{figs/gen3ex}
\medskip
...
\FIGref{fig:3genex}に示される3つの発電機で構成される電力系統モデルを考える。
...
適当な初期値を設定した場合の結果を\FIGref{fig:timeex}に示す。
...
\]
の時間微分は，式\ref{eq:Flyapeq}と同様にして
...
これまでの議論から，受動送電条件が電気サブシステム$G$が受動的であるための十分条件であることが示された。
...
\end{align}
のように展開する。
ここで，$\mathds{1}$は$\delta^{\rm lin}$の共通成分を表す基底ベクトルであり，$W$はそれ以外の偏差成分を表す基底ベクトルをまとめた行列である。
...
なお，厳密な証明は必ずしも容易ではないが，$B$と$C$のランクは特別な状況を除き$(N-1)$以上であるため，それらを前提としても現実的な解析に支障はない。
}。
ここで，$G_{\rm e}$の可観測性から
...
定義\ref{def:passivelin}の受動性と定義\ref{def:trpf}の正実性が等価であることが知られている
\footnote{
