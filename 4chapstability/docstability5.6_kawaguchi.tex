...
\]
と表される。
ここで，$f(x)$と$x$の第$i$要素を$f_i(x)$と$x_i$として，$\tfrac{\partial f}{\partial x}(x)$は$\tfrac{\partial f_i}{\partial x_j}(x)$を第$(i,j)$要素にもつ\emph{ヤコビ行列}（Jacobian matrix）\index{ やこびぎょうれつ@ヤコビ行列}である。
このヤコビ行列を用いて
\[
...
電力系統工学では，定常値において発電機の有効電力を回転子偏角で偏微分した値を\textbf{同期化力係数}（synchronizing power coefficient）\index{ どうきかりょくけいすう@同期化力係数}と呼ぶ\cite[8.4節]{kato2017electric}。
...
電力系統工学では，このような微小変動に対する安定性は，\textbf{定態安定性}（small signal stability）\index{ ていたいあんていせい@定態安定性}と呼ばれる。
...
発電機や送電線の定数は例\ref{ex:Kronode}と同じ値に設定して，\TABref{table:gensteady}に示される定常値の近傍で式\ref{eq:lindynu0}の近似線形モデルを導出する。
...
\]
の2通りを考える。
その他の定数は\TABref{table:genparams}の値を設定する。
...
なお，内部電圧の定常値は\TABref{table:gensteady}の値から変化させない。
...
比較を行うパラメータ設定を\TABref{table:parasetcom}にまとめる。


\TABref{table:parasetcom}の(a)--(d)のそれぞれの場合において，パラメータ$(\theta_1,\theta_2)$を変化させて近似線形モデルの安定性を数値的に解析してみよう。
...
\mathcal{V}_{\lambda}:= \sfker (\lambda I -A)
\]
を$\lambda$に対する\emph{固有空間}（eigenspace）\index{ こゆうくうかん@固有空間}と呼ぶ。
固有値$\lambda$に対するすべての線形独立な固有ベクトルを$v_1,\ldots,v_k$とするとき
\[
\mathcal{V}_{\lambda} = \sfspan\{v_1,\ldots,v_k\}
\]
...
任意の初期値に対して，内部状態が式\ref{eq:eqset}の平衡点集合$\mathcal{M}$のいずれかの点に収束するとき，近似線形モデルは\textbf{定態安定}\index{ ていたいあんてい@定態安定}であると呼ぶ。
%\footnote{
...
\begin{定義}[正方行列の安定性]
\label{def:matsta}
正方行列$A$に対して，そのすべての固有値の実部が負であるとき，$A$は\textbf{安定}（stable）\index{ あんてい@安定}であると呼ぶ。
\end{定義}
...
\end{align}
とする。
本書では，このサブシステムを\textbf{機械サブシステム}\index{ きかいさぶしすてむ@機械サブシステム}と呼ぶ。
...
\end{align}
とする。
このサブシステムを\textbf{電気サブシステム}\index{ でんきさぶしすてむ@電気サブシステム}と呼ぶ
\footnote{
...
\forall t \geq 0
\end{align}
を満たす半正定行列$P$が存在するとき，$\Sigma$は\textbf{受動的} （passive）\index{ じゅどうてき@受動的}であると呼ぶ。
特に，上記の半正定行列に加えて
...
\forall t \geq 0
\end{align}
を満たすある正定数$\rho$が存在するとき，$\Sigma$は\textbf{強受動的}（strictly passive）\index{ きょうじゅどうてき@強受動的}であると呼ぶ。
\end{定義}

定義\ref{def:passivelin}における関数$W(x)$は，一般に\textbf{蓄積関数}（storage function）\index{ ちくせきかんすう@蓄積関数}と呼ばれる。
なお，式\ref{eq:conosplin}の不等式は，関数$W(x)$で表されるある種のエネルギーが，式\ref{eq:conpvlin}と比較して，出力の2乗に比例する項だけ多く消散するような受動性となっている。
このような受動性は，より厳密には\textbf{出力強受動性}（output-strict passivity）\index{ しゅつりょくきょうじゅどうせい@出力強受動性}と呼ばれる。
...
\]
を$M$の$M_{22}$に関する\textbf{シューア補行列}（Schur complement）\index{ しゅーあほぎょうれつ@シューア補行列}と呼ぶ。
同様に
\[
...
式\ref{eq:sysmats}のシステム行列$(L,A,B,C)$について，つぎの3つの条件をまとめて\textbf{受動送電条件}\index{ じゅどうそうでんじょうけん@受動送電条件}と呼ぶ
\footnote{
...
\end{align}
が成り立つことがわかる。
これは，システム制御工学において\textbf{可観測性}（observability）\index{ かかんそくせい@可観測性}と呼ばれる性質である。
...
\begin{COLUMN}
\noindent \textbf{可観測性}：
式\ref{eq:siglin}の線形システム$\Sigma$について，出力$y(t)$が恒等的に0であるならば，内部状態$x(t)$も恒等的に0であるとき，$\Sigma$は\textbf{可観測}（observable）\index{ かかんそく@可観測}であると呼ばれる。
...
各々すべての初期値$x(0)$に対して，ある時刻$T > 0$において $x(T) = 0$ となる入力$u(t)$が存在するとき，$\Sigma$は\textbf{可制御}（controllable）\index{ かせいぎょ@可制御}であると呼ぶ。
...
である。
ただし，$n$は状態の次元である。
また，$\sfim$は行列の\textbf{値域}（image）\index{ ちいき@値域}を表す。
...
このような関数$V(x)$は，\textbf{リアプノフ関数}（Lyapunov function）\index{ りあぷのふかんすう@リアプノフ関数}と呼ばれる。
...
この基底を\textbf{標準基底}（standard basis）\index{ ひょうじゅんきてい@標準基底}と呼ぶ。
...
\]
とすれば構成できる。
このとき，積$WW^{\dagger}$は$\sfspan\{\mathds{1}\}$の直交補空間への\textbf{直交射影行列}（orthogonal projection matrix）\index{ ちょっこうしゃえいぎょうれつ@直交射影行列}となることから
\begin{align}\label{eq:psudinv}
WW^{\dagger} = I - \frac{1}{N} \mathds{1} \mathds{1}^{\sf T}
\end{align}
と表せる。
このような$W$の擬似逆行列$W^{\dagger}$は，特別に\textbf{ムーア・ペンローズの擬似逆行列}（Moore-Penrose pseudoinverse）\index{ むーあぺんろーずのぎじぎゃくぎょうれつ@ムーア・ペンローズの擬似逆行列}と呼ばれる\cite{bernstein2009matrix}。



\FIGref{fig:orthogonal}では，黒矢印により$\sfspan\{\mathds{1}\}$の空間を示し，それに直交する平面として直交補空間$\sfspan\{\mathds{1}\}^{\perp}$を示している。
...
\end{align*}
に対して，その\textbf{伝達関数}（transfer function）\index{ でんたつかんすう@伝達関数}は
\[
Q(s):=C(sI-A)^{-1}B +D
\]
と定義される。
...
伝達関数$Q(s)$のすべての極の実部が負であるとき，$Q(s)$は\textbf{安定}（stable）\index{ あんてい@安定}であると呼ぶ。
\end{定義}

伝達関数の極は分母多項式の零点である。
式\ref{eq:trGs}の$H(s)$が安定であることは，行列$\taud^{-1}A$のすべての固有値の実部が負であることに等しい。

また，伝達関数の\textbf{正実性}（positive realness）\index{ せいじつせい@正実性}はつぎのように定義される。
...
\end{align}
を定義する。
つぎの3条件が満たされるとき，$Q(s)$は\textbf{正実}（positive real）\index{ せいじつ@正実}であると呼ぶ。
\begin{itemize}
...
\]
と分解できる。
この$\tfrac{M+M^*}{2}$を$M$の\textbf{複素対称部分}（Hermitian part）\index{ ふくそたいしょうぶぶん@複素対称部分}と呼ぶ。
また，$\tfrac{M-M^*}{2}$を$M$の\textbf{複素歪対称部分}（skew Hermitian part）\index{ ふくそひずみたいしょうぶぶん@複素歪対称部分}と呼ぶ。
\end{COLUMN}
...
必要条件を導く数学的な準備として，正実性と類似した概念である伝達関数の\textbf{負虚性}（negative imaginaryness）\index{ ふきょせい@負虚性}を導入する\cite{petersen2010feedback,xiong2010negative}。

\begin{定義}[伝達関数の負虚性]
\label{def:trni}
原点に極をもたない正方な伝達関数$Q(s)$に対して，式\ref{eq:defOm0}の$\Omega_0$を定義する。
つぎの3条件が満たされるとき，$Q(s)$は\textbf{負虚}（negative imaginary）\index{ ふきょ@負虚}であると呼ぶ。
\begin{itemize}
\item $Q(s)$のすべての極の実部は非正である。
...
周波数応答関数$Q(\bm{j} \omega)$の$\omega \in \mathbb{R}$に関する軌跡を複素平面上にプロットしたものは\textbf{ナイキスト軌跡}（Nyquist curve）\index{ ないきすときせき@ナイキスト軌跡}と呼ばれる。
ナイキスト軌跡は，フィードバック系の安定性に関する幾何学的な解析にしばしば用いられる。
その解析手法は\textbf{ナイキストの安定判別法}（Nyquist stability criterion）\index{ ないきすとのあんていはんべつほう@ナイキストの安定判別法}と呼ばれる。
...
このような状態変数に時間スケールの違いがあるシステムは，システム制御工学において，\textbf{特異摂動系}（singularly perturbed system）\index{ とくいせつどうけい@特異摂動系}と呼ばれている\cite{kokotovic1987singular}。
